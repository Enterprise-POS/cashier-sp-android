...
class WriteMetadata(private val repository: DatabaseCacheMetadataRepository) {
    operator fun invoke(entity: DatabaseCacheMetadataEntity): Flow<Resource<Long>> = flow {
        emit(Resource.Loading())
        try {
            val result = repository.writeMetadata(entity)
