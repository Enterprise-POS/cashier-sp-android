...
class WriteAndReturnMetadata(private val repository: DatabaseCacheMetadataRepository) {
    operator fun invoke(entity: DatabaseCacheMetadataEntity): Flow<Resource<DatabaseCacheMetadataEntity>> = flow {
        emit(Resource.Loading())
        try {
            val result = repository.writeAndReturnMetadata(entity)
