...
class UnselectStore(private val selectedStoreDS: SelectedStoreDS) {
    operator fun invoke(): Flow<Resource<Unit>> = flow {
        try {
            emit(Resource.Loading())
            selectedStoreDS.unselectAnything()
            emit(Resource.Success(Unit))
        } catch (exception: Exception) {
            println("Exception error at UnselectStore: ${exception.message}")
